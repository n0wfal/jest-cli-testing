...
const todoDB =
  process.env.NODE_ENV === "TEST" ? "todo_test_db.json" : "todo_db.json";
...
const init = () => {
  ...
  try {
    ...
    todosLength = Object.keys(contents).length;
  } catch (error) {
    fs.writeFileSync(dbPath, `{ }`, "utf-8");
  }
};
...
// List all todos.
const listTodos = () =>
  Object.keys(contents).map((c, _i) =>
    console.log(
      `${++_i}. ${contents[c].todo} ${contents[c].done ? "\u2713" : "\u2717"}`
    )
  );
...
const interactiveMarkTodo = async () => {
  ...
  const formattedTodos = Object.keys(allTodos)
    .map((e) => {
...
const interactiveMenu = async () => {
  ...
  switch (action) {
    case "add":
      await interactiveAddTodo();
      break;
    case "list":
      await listTodos();
      break;
    case "mark":
      await interactiveMarkTodo();
      break;
    case "exit":
      successLog("Exiting.");
      break;
